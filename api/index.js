...
async function animeterbaru(page = 1) {
  const res = await axios.get(`https://cors.caliph.my.id/https://v2.samehadaku.how/anime-terbaru/page/${page}/`, { headers });
...
async function search(query) {
  const res = await axios.get(`https://cors.caliph.my.id/https://v2.samehadaku.how/?s=${encodeURIComponent(query)}`, { headers });
  const $ = cheerio.load(res.data);
  const data = [];
  $('.animpost').each((_, e) => {
    data.push({
...
async function detail(link) {
  ...
  const targetUrl = link.startsWith('http') ? link : `https://v2.samehadaku.how${link}`;
...
async function download(link) {
  ...
  for (const li of $('div#server > ul > li').toArray()) {
    ...
    try {
        const r = await axios.post('https://cors.caliph.my.id/https://v2.samehadaku.how/wp-admin/admin-ajax.php', body, {
        headers: {
            ...headers,
